...
class MultiPacmanAgent(MultiAgentSearchAgent):
    def minimax(self, currentGameState, depth, agent):
        ...
        if agent == 0:
            ...
            for action in currentGameState.getLegalActions(agent):
                eval = self.minimax(currentGameState.generateSuccessor(agent, action), depth - 1, agent + 1)[0]
                if(eval > maxEval):
                    maxAction = action
                    maxEval = eval
    ...
    def eval(self, currentGameState):
        ...
        foodbits = currentGameState.getNumFood()
        ...
        return 1.0/(foodbits + 1) * foodLeftMultiplier + ghostDist + 1.0/(fooddist + 1) * foodDistMultiplier + 1.0/(capsulesLeft + 1) * capsLeftMultiplier
